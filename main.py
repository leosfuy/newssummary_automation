function sendDailyStockNews() {
  var apiKey = 'AIzaSyDIeCrGR1qxOwRonJ_Gtkoforabt2POprk'; 
  var emailAddress = 'leeangel100230@gmail.com'; 

  try {
    // === 1. 使用 Google 新聞搜尋「類股」，並限制「1天內 」 ===
    ...
    for (var i = 0; i < Math.min(30, entries.length); i++) {
      ...
      var desc = entries[i].getChildText('description') || ""; //從XML中提取標題及內文
      
      newsList += "【" + sourceName + "】標題：" + title + "\n摘要細節：" + desc + "\n\n";
      addedCount++;
    }

    Logger.log("成功抓取 " + addedCount + " 篇 Google 類股新聞");
    if (addedCount === 0) throw new Error("今日沒有相關的類股新聞。");

    // === 2. 把提取的資訊打包成JSON丟給 AI (生成摘要) ===
    ...
    console.log("<div style='font-family: \"Helvetica Neue\", Helvetica, Arial, sans-serif; line-height: 1.8; font-size: 15px; color: #333;'>" + cleanSummary + "</div>");
    MailApp.sendEmail({
      to: emailAddress,
      subject: subject,
